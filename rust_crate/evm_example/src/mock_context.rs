...
impl MockContextBuilder {
    ...
    pub fn with_caller_hex(mut self, hex_str: &str) -> Result<Self, String> {
        ...
        if clean_hex.len() != 40 {
            return Err(format!(
                "Caller address must be 40 hex characters, got {}",
                clean_hex.len()
            ));
        }
    ...
    pub fn build(self) -> MockContext {
        ...
        println!(
            "Created MockContext with original code length: {} bytes, prefixed length: {} bytes",
            self.contract_code.len(),
            self.contract_code.len() + 4
        );
...
impl MockContext {
    ...
    fn create_prefixed_code(wasm_code: &[u8]) -> Vec<u8> {
        ...
        println!(
            "Created prefixed code: length prefix = {:02x?}, original length = {}",
            &code_length.to_be_bytes(),
            code_length
        );
    ...
    pub fn set_storage(&self, key: &str, value: Vec<u8>) {
        ...
        println!(
            "Storage store: key={} (normalized: {}), value={}",
            key,
            normalized_key,
            hex::encode(&storage_value)
        );

        self.storage
            .borrow_mut()
            .insert(normalized_key, storage_value);
    }
    ...
    pub fn set_storage_bytes32(&self, key: &str, value: [u8; 32]) {
        ...
        println!(
            "Storage store (bytes32): key={} (normalized: {}), value={}",
            key,
            normalized_key,
            hex::encode(&value)
        );
        ...
        self.storage
            .borrow_mut()
            .insert(normalized_key, value.to_vec());
    }
    ...
    pub fn get_storage(&self, key: &str) -> Vec<u8> {
        ...
        match storage.get(&normalized_key) {
            Some(value) => {
                println!(
                    "Storage load: key={} (normalized: {}), value={}",
                    key,
                    normalized_key,
                    hex::encode(value)
                );
            ...
            None => {
                ...
                println!(
                    "Storage load: key={} (normalized: {}), value=<zero>",
                    key, normalized_key
                );
                zero_value
            }
    ...
    fn normalize_storage_value(&self, value: Vec<u8>) -> Vec<u8> {
        ...
        if value.len() != 32 {
            println!(
                "Storage value normalized: original_len={}, normalized_len=32",
                value.len()
            );
        }
    ...
    pub fn consume_gas(&mut self, amount: i64) -> bool {
        ...
        println!(
            "Consumed {} gas, success={}, remaining={}",
    ...
    pub fn copy_call_data(&self, dest: &mut [u8], data_offset: usize, length: usize) -> usize {
        ...
        if copy_len > 0 {
            ...
            println!(
                "Copied {} bytes of call data from offset {} to buffer",
                copy_len, data_offset
            );
        } else {
            println!(
                "No call data copied: offset={}, length={}, total_data_len={}, dest_len={}",
        ...
        if copy_len < dest_len && copy_len < length {
            ...
            if zero_fill_len > 0 {
                ...
                println!(
                    "Zero-filled {} bytes in call data destination buffer",
                    zero_fill_len
                );
            }
        }
    ...
    pub fn copy_code(&self, dest: &mut [u8], code_offset: usize, length: usize) -> usize {
        ...
        if copy_len > 0 {
            dest[..copy_len]
                .copy_from_slice(&self.contract_code[code_offset..code_offset + copy_len]);
            println!(
                "Copied {} bytes of code from offset {} to buffer",
                copy_len, code_offset
            );
        } else {
            println!(
                "No code copied: offset={}, length={}, total_code_len={}, dest_len={}",
    ...
    pub fn register_contract(&mut self, address: [u8; 20], name: String, code: Vec<u8>) {
        ...
        println!(
            "📝 Registered contract '{}' at address 0x{}",
            name,
            hex::encode(&address)
        );
    }
...
impl EvmContext for MockContext {
    ...
    fn emit_event(&self, event: LogEvent) {
        ...
        println!(
            "Emitted event #{}: contract=0x{}, topics={}, data_len={}",
            event_count + 1,
...
impl ContractCallProvider for MockContext {
    fn call_contract(
        ...
        let (target_code, contract_name) = match self.get_contract_info(target) {
            Some(info) => {
                println!(
                    "   📋 Found target contract: '{}', code length: {} bytes",
                    info.name,
                    info.code.len()
                );
                (info.code, info.name)
            ...
            None => {
                ...
                println!(
                    "   📋 Using current contract code: {} bytes",
                    current_code.len()
                );
            ...
            *current_value,
            ...
            Ok(result) => {
                ...
                if result.success && !result.is_reverted {
                    ...
                    println!(
                        "   ❌ DELEGATECALL failed or reverted: {:?}",
                        result.error_message
                    );
                    ContractCallResult::failure(result.return_data, gas_used)
                }
    ...
    fn create_contract(
        ...
        let return_data = if !data.is_empty() {
            ...
            println!(
                "   🔧 Executing constructor with {} bytes of code",
                code.len()
            );
            println!(
                "   🔧 Executing constructor with {} bytes of data",
                data.len()
            );
